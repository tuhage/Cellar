...
struct HistoryEvent: Identifiable, Codable, Hashable, Sendable {
    ...
    static var preview: HistoryEvent {
        HistoryEvent(
            timestamp: Date(),
            eventType: .installed,
    ...
    static var previewList: [HistoryEvent] {
        let now = Date()
...
enum HistoryEventType: String, Codable, Sendable, CaseIterable {
    case installed
    case uninstalled
    case upgraded
    case serviceStarted
    ...
    /// A SwiftUI color associated with the event type.
    var color: Color {
        switch self {
